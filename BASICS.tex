...
\end{equation}
\noindent whith diagonal entries $S_{\mu\mu} = 1$, and off-diagonal elements $0 < S_{\mu\nu} < 1$. The eigenvalue problem for RHF then takes the more general form
\begin{equation}
...
The SCF procedure is summmarized in Algorithm \ref{algo:SCF}. At every iteration, the Fock matrix is diagonalized to obtain a new guess for the density matrix, which is used for constructing the Fock matrix in the next step. These steps are repated until \emph{self-consistency} is reached. There are different ways to test for convergence, the simplest being the Hartree-Fock energy difference between subsequent iterations. A more rigourous bound is given by the matrix norm of the error vector $\mbf{e}$
\begin{equation}
...
There is no guarantee that the SCF procedure converges. For small molecules and equilibrium geometries, the unmodified SCF procedure converges quite smoothly. For large, diffuse basis sets or distorted gemoetries, additional modifications to the algorithms might be necessary:
\begin{enumerate}
\item Direct inversion of the iterative subscpace (DIIS): the previous Fock matrices are used for extrapolation to generate a better Fock matrix (Annex)
...
The Fock matrix can be divided into four blocks: occupied-occupied, occupied-virtual, virtual-occupied and virtual-virtual (Figure ...). The terms "particle" (p) and "hole" (h) may be used instead of occupied and virtual. In the special case where the orbitals are the canonical MOs, the Fock matrix is diagonal. This is known as the \emph{canonical condition} for the HF wavefunction. However, diagonality is not necessary for obtaining a valid HF wavefunction. The general Hartree-Fock equations take the form
\begin{equation}
\mbf{f} \phi_P = \sum_{PR} \lambda_{PR} \phi_R
\end{equation}
...
\begin{equation}
f_{IA} = \bra{HF}\mbf{f}\ket{a\pdg_A a_I HF} = 0
\end{equation}
\noindent and are zero by virtue of \emph{Brillouin's theorem}. This is also known as the \emph{variational condition} for the HF wavefunction; it is an alternative formulation of the variational principle (ref, section 10.3.5).

The Hartree-Fock energy is therefore stable under unitary rotations
\begin{equation}
\phi_{I'} = \mbf{U} \phi_I \qquad U\pdg U = \mbf{U}
\end{equation}
\noindent The canonical MOs may be rotated to give another orbital representation with smaller spatial extent, also known as localized molecular orbitals (see chapter ...).

\section{Electron Correlation}

In the Hartree-Fock method, the electron-electron interaction is replaced by an average interaction. For large basis sets, Hartree-Fock is actually able to recover approximately 99\% of the electron correlation. Unforntunately, the remaining 1\% are often important to compute chemical properties with suffcient accuracy.

Electron correlation arises from electrons trying to avoid each other due to coulombic repulsion (coulomb correlation) and the antisymmetry principle (Fermi correlation). This in turn leads to a region of space around each electron where the probabilty of finding another electron is reduced, typically known as the \emph{coulomb hole} for electrons of opposite spin, and \emph{Fermi hole} for electrons of the same spin. 

Another distinction is often drawn between \emph{dynamic correlation} and \emph{static correlation}, although there exists no formal definition. Dynamic correlation generally describes the correlated movement of electrons due to their "instantaneous mutual repulsion" (ref). For example, in the ground-state Helium atom, electron correlation is purely dynamical. Static (or non-dynamic) correlation on the other hand arises in the case of near-degeneracy, where multiple configurations of similar energy contribute to the ground-state wave function. To show the difference between these two two types of correlation, the dissociation of the H$_2$ molecule is often taken as an illustrative example. At equilibrium distance, correlation is mostly dynamic, and the ground-state can be well described as a singlet state. In the (covalent) dissociation limit, electrons may be coupled to yield either a singlet \emph{or} a triplet, as the energy difference between those two states vanishes (Figure). The correlation in the system in then entirely static. There is no clear-cut line between dynamic and static correlation, but it offers a useful classification for correlation effects.

To fully capture both dynamic and static correlation, it is crucial to add additional Slater determinants as mentioned previously. Methods which generate excited SLater determinants from a single reference SD recover mostly dynamical correlation. """" Electrons are disturbed through their repulsion and are excited to higher spin orbitals, hence the need of additional excited SDs """". Methods generating SDs from multiple references recover mostly static correlation. At the theoretical limit (i.e. the full configuration space), both types eventually recover all correlation effects. It is therefore the responsibility of the computational chemist to choose the correct method that best fits the problem at hand.

Any electronic structure method that improves on the HF wavefunction is usually referred to as a "correlated method". Hartree-Fock, by convention, is an "uncorrelated method", although it does include electron correlation. 
...
Configuration interaction (CI) is the simplest and oldest example of a correlated electronic structure method. The CI wave function takes the general form
\begin{equation}
...
\begin{equation}
\frac{\partial}{\partial C_i} \frac{\bra{CI}\mbf{H}\ket{CI}}{\bra{CI}\ket{C}}
\end{equation}
...
\end{bmatrix}
\end{equation}
\noindent In analogy to the Fock matrix, some blocks in the CI matrix are zero. Brillouin's theorem states that there is no coupling between the ground state and the singlet states. That does not signify that they do not contribute to the Ci energy at all. Singles mix \emph{indirectly} via doubles. Moreover, matrix blocks of the Hamiltonian between two SDs which differ by more than two spin orbital orbitals are also zero. Triples mix with doubles and singles, but not with the ground state.
...
Full CI, i.e. including all excitation manifolds, is only computationally feasible for all but the smallest molecules, due to the binomial increase in SDs as s function of system and basis set size. For this reason, the CI wave function my be truncated at a given excitation level. Including only singles gives configuration interaction with singles (CIS), including singles and doubles yields configuration interaction with singles and doubles (CISD), etc. Higher order methods recover more of the correlation energy, but come at a higher computational cost.
...
E(A+B) = E(A) + E(B)
\end{equation}
\noindent \emph{Size-extensivity} is closely-related criterium that states that the energy should be a linear function of the number of electrons (ref), i.e. the energies of small and large molecules have similar errors, which is important for comparing properties. As the system size increases, truncated CI recovers less and less of the total correlation energy.

% Computational Chemistry: A Practical Guide for Applying Techniques to Real https://books.google.de/books?id=-pn8K53IUqgC&pg=PA223&lpg=PA223&dq=size+consistency+vs+extensivity&source=bl&ots=x_g9YAUAPZ&sig=ACfU3U3bLZ64HmFbRQrFiSvYR1BwsHOdyw&hl=en&sa=X&ved=2ahUKEwjag4H696XyAhVhgf0HHbVfBzEQ6AF6BAguEAM#v=onepage&q=size%20consistency%20vs%20extensivity&f=false
...
\begin{equation}
a\pdg_I a\pdg_J + \sum_{A > B} t_{IJ}^{AB} a\pdg_A a\pdg_B = \left( 1 + t_{IJ}^{AB} \mbf{tau}_{IJ}^{AB} \right) a\pdg_I  a\pdg_J 
\label{eq:epair}
\end{equation}
\noindent where $\mbf{t}$ are the associated cluster coefficients, also known as \emph{amplitudes}. %, and $\mbf{\tau}$ are the cluster operators. 
By virtue of Brillouin's theorem, single excitations are not considered. Equation \ref{eq:epair} is known as the \emph{electron pair}, \emph{two-electron cluster} or \emph{pair-cluster} approximation. 

In a first approximations, electron pairs may be treated indepedently in a molecular system, in what is known as the independent electron pair approximation (IEPA). The total correlation energy is then simply given as the sum of the individual \emph{pair correlation energies}
\begin{align}
E_{corr}^{IEPA} &= \sum_{I < J} e_{IJ}  \\
\ket{IEPA} = \sum_{I < J, A < B} \left(1 + t_{IJ}^{AB}\mbf{\tau}_{IJ}^{AB} \right) \ket{HF} 
\end{align}
\noindent A more complete picture of electron correlation is given by additionally letting electron clusters interact whith each other by using the parametrization
\begin{equation}
\ket{CCD} = \left( \prod_{A > B,I > J} 1 + t_{IJ}^{AB}\mbf{\tau}_{IJ}^{AB} \right) \ket{HF}
\end{equation}
\noindent The resulting wave function correpsonds to the coupled cluster approximation including only doubles (CCD). As opposed to CID, CCD additionally includes \emph{products} of doubles cluster operators ($\tau_{IJ}^{AB} \tau_{KL}^{CD}$, $\tau_{IJ}^{AB} \tau_{KL}^{CD} \tau_{MN}^{EF}$), in other words, doubles excitations are included up to infinite order (ref). It is this property that makes CC size-extensize.

% ref R J Bartlett J Phys Chem 93 (1996) 216

\subsection{Coupled Cluster Ansatz}

The CCD model can be generalized to let custers of three and more electrons interact with each other, and electrons interact within these clusters. The general CC ansatz reads
\begin{equation}
\ket{CC} = \left( \prod_{\mu} 1 + t_{\mu}\boldsymbol{\tau}_{\mu} \right) \ket{HF} = exp(t_{\mu}\boldsymbol{\tau}_{\mu}) \ket{HF} = exp(\mbf{T}_{\mu}) \ket{HF}
\label{eq:CCANSATZ}
\end{equation}
\noindent where $\mbf{T}_{\mu}$ is the \emph{cluster operator}, and $\mu$ are the excitation manifolds. The cluster operator may be partitioned into classes comprising all singles, doubles, ... excitations:
\begin{equation}
\mbf{T} = \mbf{T}_1 + \mbf{T}_2 + ... \mbf{T}_N
\end{equation}
\noindent Truncating the cluster operator to include only excitations up to a certain degree yields a hierarchy of CC method named CCS, CCSD, CCSDT etc. Again, including higher orders necessitate a higher computational effort. The exponential in Equation \ref{eq:CCANSATZ} for different truncation levels is approximated as
\begin{align}
exp(\hat{T}) &= \hat{T}_1 \\
...
\begin{equation}
\mbf{\ovl{H}} = exp(-\hat{T}) \mbf{H} exp(\hat{T})
\end{equation} 
\noindent gives the set of non-linear equations
\begin{align}
\bra{\mu_1}\ovl{H}\ket{HF} &= 0 \\
\bra{\mu_2}\ovl{H}\ket{HF} &= 0 \\
\bra{\mu_3}\ovl{H}\ket{HF} &= 0 \\
\vdots
\label{eq:CCEQ}
\end{align}
...
Baker–Campbell–Hausdorff (BCH) formula (ANNEX?), but will not be discussed in detail here. As an example of the exact form of the working equations, consider the CCSD model truncated at doubles excitation. Equation \ref{eq:CCEQ} then reduces to
\begin{align}
\sbra{\mu_1} \hat{\ovl{H}} + \left[\hat{\ovl{H}},\hat{T}_2\right] \sket{HF} = 0 
\label{eq:CCSDEQSINGLES}
\\
\sbra{\mu_2} \hat{\ovl{H}} + \left[\hat{\ovl{H}},\hat{T}_2\right] + \frac{1}{2} \left[ \left[ \hat{\ovl{H}},\hat{T}_2\right],\hat{T}_2\right] \sket{HF} = 0
...
Coupled cluster and configuration interaction offer a systematic way to move towards the exact solution to the Schrödinger equation by means of adding more Slater determinants. However, the calculation of the  CC and CI wavefunctions is very expensive, and it may be profitable to look at alternative schemes. \emph{Pertubation theory} (PT) is a different approach to systematically close in on the exact wavefunction. It is based on the idea that the exact solution differs only slightly from a previously solved problem for a simpler, but related system.
...
\label{eq:RSPTSWF}
\end{equation}
\noindent Furthermore, to obtain simpler expressions for $E_i^{(k)}$, the normalization is chosen such that $\sbra{\Psi_i^{(0)}}\sket{\Phi_i}=1$, also known as intermediate normalization. From this it follows that the approximate wave functions are then orthogonal to the reference states
\begin{equation}
...
\end{equation}
\noindent Multiplying from the left by $\sbra{\Psi_n^{(0)}}$, the expansion coeffcients can be obtained with
\begin{equation}
...
\begin{equation}
\hat{U} = \hat{H} - \hat{f} = \sum_{PQRS} \hat{g}_PQRS a\pdg_P a\pdg_R a_S a_Q - \hat{V}^{HF} 
\end{equation}
...
\label{eq:UMP2}
\end{equation}
\noindent For a closed-shell molecule, using spin-separation similarly to how it was done in Section 1.4.3 for Hartree-Fock, the restricted MP2 energy is given as
\begin{equation}
\begin{split}
...
\end{equation}
\noindent The energy expressions for MP3 and beyond will not be given here.

\subsection{Convergence Behaviour of the MP$n$ series}
...
Ideally, the MP$n$ energy should converge monotonically towards the limit with increasing order of perturbation. However, such a behaviour is not guaranteed. Contrary to the CI enery, which is dtermined variationally and therefore has a lower bound, the same is not true for MPPT and the MP$n$ series may become divergent or oscillating for larger basis sets, especially if diffuse functions are used. MP2 improves on the HF wavefunction, but slightly overestimates correlation energy. MP3 underestimates electron correlation, and porperties computed at this level are often inferior to those computed at second order. MP4 again overestimates correlation effects, but is better than MP2.

Due to the erratic convergence behaviour of MPPT, higher order variations like MP3 or MP4 have fallen somewhat out of favour in the recent years. Moreover, the requirement of the single-determinant HF wave function being a suitable starting guess makes MPPT ill-suited to describe static correlation effects.  

\subsection{Spin-Component-Scaled M{\o}ller Plesset Perturbation Theory}

With the rise pf density functional theory (DFT), even the computationally inexpensive MP2 method fell out of use in favour of DFT which often shows better performance and accuracy for the same molecular systems.

In the early 2000s, MPPT again gained more popularity with the introdution of \emph{spin-component scaling} (SCS) by Grimme et al. (Gri2003,Gri2012) which greatly improves on the accuracy of MP2.

Consider again the MP2 energy in the unrestricted case in Eqaution \ref{eq:RMP2}. The energy contributions can be split into same-spin (SS) and opposite spin (OS) components:
\begin{align}
...
\end{align}
The correlation effects in SS and OS are of different nature as discussed in Section .... Hartree-Fock accounts for Fermi correlation by the antisymmetry principle but does not fully account for Coulomb correlation. MP2 cannot fully rectify this deficiency in the starting guess. SCS-MP2 accounts for this behaviour by scaling down the SS components and scaling up the OS components
\begin{equation}
E_{SCS-MP2} = c_{os} E_{OS-MP2} + c_{ss} E_{SS-MP2}
\end{equation}
\noindent where the scaling factors are determined empricially by fitting to a data set, with $c_{ss}$ = 6/5 and $c_{os}$ = 1/3. In later iterations of SCS-MP2, the two parameters were unified by introducing the relationship
\begin{equation}
c_{ss} = 4 - 3c_{os}
\end{equation}
\noindent whith $c_{ss}$ and $c_{os}$ set to 0.4 and 1.3 respectively.

SCS-MP2 gives considerable improvements to reaction energies (16), barrier heights (40,41), geometries and vibrational frequencies (42), comparable to QCISD(T) with errors on the order of 1.7 kcal/mol. Strictly speaking, SCS-MP2 is no longer an \emph{ab intio} method, but \emph{semi-empirical}. 

SCS was initially an \emph{ad-hoc} improvement to the description of the wave function, but it is possible to justify its position in the theoretical framework of MPPT (23,28).  

Over the years, many different variations of SCS-MP2 have been proposed (19,20,24,25). One variation is the so-called spin-opposite scaled (SOS) MP2 method (19), where the SS components are simply ignored:
\begin{equation}
E_{SOS-MP2} = c_{os} E_{OS-MP2}
\end{equation}
\noindent with $c_{os}$ set to 1.3 instead of 1.2 as in SCS-MP2. The method can be justified by observing that the SS already do not contribute a lot to the SCS-MP2 energy. SOS-MP2 has comparable or slightly worse accuarcy than SCS-MP2. The major advantage which makes SOS-MP2 one of the more attractive spin-component scaling variants is the reduced scaling $\ccpx{4}$ compared to $\ccpx{5}$ for (SCS)-MP2. 

% Gri2003 Grimme S. Improved second-order Møller—Plesset perturbation theory by separate scaling of parallel- and antiparallel-spin pair correlation energies. J Chem  Phys 2003, 118: 9095—9102

% Gri2012 WIREs Comput Mol Sci 2012. doi: 10.1002/wcms.1110

% within that: 16, 40, 41, 42
...
\hat{H} = \hat{F} + \hat{U}
\end{equation}
\noindent where $\hat{U}$ is also known as the \emph{fluctuation potential}, the coupled cluster singles doubles method by Christiansen et al. (ref) approximate the doubles amplitudes to first order only. The doubles equation \ref{eq:CCSDEQDOUBLES} thus becomes
\begin{equation}
...
The CC2 energy has a similar accuracy and computational effort to MP2. Spin-component scaling has been generalized to CC2 as well (1,2), with similar improvements to its accuracy (3). 

Higher order CC methods like CCSDT can be approximated in a similar way to obtain the approximate CC3 method (4), where triples contributions are approximated to second order. A related method is the coupled cluster singles doubles with perturbative triples method (5), abbreviated as CCSD(T), where triples contributions are treated perturbatively and \emph{added} to the CCSD model. CCSD(T) and CC3 have comparable accuracy and cost. 

% 1 DOI: 10.1039/b803727b
% 2 DOI: 10.1063/1.3584177 
% 3 https://doi.org/10.1021/acs.jctc.9b00676
% 4 https://doi.org/10.1063/1.473322
...
The previous sections give insight into the most popular, single-determinant, correlated electronic structure methods. The methods vary widely in accuracy, computaional cost and convergence behaviour. 

Table ... gives the formal computational scaling for some of the methods. The notation $\mathcal{O}()$, also known as \emph{Big O notation}, is the standard way of indicating the limiting behaviour of algorithms for increasing input size. $N$ is used as a measure of the molecular system size (e.g. number of atoms or basis functions). \emph{Formal} scaling means that factors like sparsity or locality are not considered.
...
At the time of writing, on current work stations, $\ccpx{5}$ methods are limited to system sizes of around 100 atoms $\ccpx{6}$ to sizes of several tens of atoms. Models with $\ccpx{7}$ scaling and beyond are not used routinely. 

In terms accuracy, the current trend is often observed (ref Frank):
...
%\subsection{Diagrammatic Perturbation Theory}

\section{Green's Functions (?)}

\section{Excited States}

\section{Coupled Cluster Linear Repsonse}
