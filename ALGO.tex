...
\caption{Davdison-Liu Algorithm. Indices $i,j,k...$ implicitly loop over the number of roots $n_{roots}$, and indices $\oli, \olj, \olk, ...$ implicitly loop over the whole Davidson subspace $n_{dav}$.}
\label{algo:DAVIDSON}
\end{algorithm}
...
\caption{Modified Davdison algorithm with DIIS acceleration for pseudo-eigenvalue problems.}
...
All calculations in this report use the robust minimax approximation as proposed by Helmich-Paris and Visscher \cite{Hel2016}. They have published their source code on Github (github.com/bhelmichparis/laplace-minimax), which was in turn incorporated into the MEGALOchem software.
